...
[![Version](https://img.shields.io/badge/version-1.4.0--beta.3-orange)](CHANGELOG.md)
...
Steam Workshop is the stable channel. Public beta builds are published as
[GitHub pre-releases](https://github.com/ampynjord/ad-astra-stellaris/releases):
install them as a separate local mod and never enable the beta and Workshop
copies together. See [Testing the public beta](docs/BETA.md).
...
## Roadmap — one mod, a wider ecosystem

The roadmap is direction, not a release calendar. A version is released only
after its targeted in-game tests pass. **Ad Astra: Origins** remains the only
Ad Astra mod: it covers the ten historical ages, the first space program and
emergence into the galaxy.

**1.4 “Galileo”** remains the immediate priority: research-led progression,
...
Ad Astra depends on no other mod and promises compatibility with none.
Whatever the project may need one day - scale, stations, worlds - will be
built in-house, if and when it earns its place.

Nomadic arks, generation ships, Gestalts, and deeper planetary politics are
long-term ideas for Origins only. They are not separate modules, releases, or
promises; a prototype must first prove its value, performance, and distinct
place beside the existing Stellaris mod ecosystem.
...
Created by **ampynjord**. Released under the [Ad Astra Proprietary License](LICENSE).
Personal use of official releases is welcome; reuploads, mirrors, derivative
releases, and reuse of original project assets are not permitted.
